...
while True:
    for i in tasks_file.find({'status':1}):
        if 1000*time.time()-i['finishtime']>604800000:
            ...
            sendsms3(i['title'],"已经超过评审期并关闭，您的最终得分为" + str(task_score),i['tasker_main'],i['tasker_main'],users_db.find_one({'username': i['tasker_main']})['mobile']) #主负责人
            for tasker in i['task_other']:
                sendsms3(i['title'], "已经超过评审期并关闭，您的最终得分为" + str(task_score * 0.5), i['tasker_main'], tasker, users_db.find_one({'username': tasker})['mobile'])  # 其他负责人
    time.sleep(1200)